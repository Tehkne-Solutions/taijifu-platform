...
const all=sets.flatMap(([,cfg])=>cfg);if(new Set(all.map(x=>x.id)).size!==96)errors.push('duplicate nucleus across published slices');if(names.length!==128||paths.length!==32||belts.length!==10)errors.push('canon structure mismatch');const nodes=all.length*3,edges=nodes+(nodes-1);if(nodes!==288)errors.push(`content_nodes=${nodes}, expected=288`);if(edges!==575)errors.push(`content_edges=${edges}, expected=575`);if(errors.length){console.error('TAIJIFU_CONTENT_VALIDATION=FAIL');for(const e of errors)console.error('- '+e);process.exit(1)}console.log('TAIJIFU_CONTENT_VALIDATION=PASS');console.log('published_belts=7');console.log(`content_nodes=${nodes}`);console.log(`content_edges=${edges}`);console.log('published_nuclei=96');console.log('coverage=lesson+quiz+guided-practice per nucleus');
