...
const config=[...whiteConfig,...yellowConfig];
const nodes:any[]=[];
for(const item of config){
  ...
  nodes.push({id:`CONTENT-${n.code}-LESSON`,...common,type:"lesson",title:`${n.code} · ${n.name}`,summary:item.summary,durationMinutes:12,body:{learningObjective:item.summary,sections:[{kind:"concept",title:"Por que importa",text:item.summary},{kind:"principle",title:"Regra de prática",text:"Priorize controle, percepção e função antes de velocidade ou intensidade."},{kind:"safety",title:"Safety",text:safety}]}});
