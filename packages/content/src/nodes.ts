...
const config=[...whiteConfig,...yellowConfig,...orangeConfig,...redConfig,...greenConfig,...cyanConfig];
const nodes:any[]=[];
for(const item of config){
  ...
  nodes.push({id:`CONTENT-${n.code}-LESSON`,...common,type:"lesson",title:`${n.code} · ${n.name}`,summary:item.summary,durationMinutes:12,body:{learningObjective:item.summary,sections:[{kind:"concept",title:"Por que importa",text:item.summary},{kind:"principle",title:"Regra de prática",text:"Priorize controle, adaptação, função e capacidade de parar antes de velocidade ou intensidade."},{kind:"safety",title:"Safety",text:safety}]}});
