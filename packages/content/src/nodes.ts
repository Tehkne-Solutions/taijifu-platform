...
const config=[...whiteConfig,...yellowConfig,...orangeConfig,...redConfig,...greenConfig,...cyanConfig,...blueConfig];
const nodes:any[]=[];
for(const item of config){
  ...
  nodes.push({id:`CONTENT-${n.code}-LESSON`,...common,type:"lesson",title:`${n.code} · ${n.name}`,summary:item.summary,durationMinutes:12,body:{learningObjective:item.summary,sections:[{kind:"concept",title:"Por que importa",text:item.summary},{kind:"principle",title:"Regra de prática",text:"Priorize qualidade sustentável, recuperação, critérios de redução e capacidade de parar antes de volume ou intensidade."},{kind:"safety",title:"Safety",text:safety}]}});
  nodes.push({id:`CONTENT-${n.code}-QUIZ`,...common,type:"quiz",title:`Checkpoint · ${n.code}`,summary:`Verificação conceitual do Núcleo ${n.code}.`,durationMinutes:4,questions:[{id:`CONTENT-${n.code}-QUIZ-Q1`,prompt:`Qual é o foco principal de ${n.name}?`,kind:"single-choice",options:[item.summary,"Executar até a exaustão","Ignorar sinais de degradação"],correctIndex:0},{id:`CONTENT-${n.code}-QUIZ-Q2`,prompt:"O que prevalece se a qualidade ou a condição segura degradar?",kind:"single-choice",options:["Aumentar a carga","Reduzir, modificar ou parar","Manter o plano original a qualquer custo"],correctIndex:1}]});
