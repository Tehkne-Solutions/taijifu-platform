...
out.push(h(2, "7. PFI — Preparação Física Integrada"));
out.push(`${pfi.pfiSystem.principle}\n`);
out.push(h(3, "Objetivos"));
out.push(bullets(pfi.pfiSystem.objectives));
out.push(h(3, "Capacidades"));
for (const c of pfi.pfiSystem.capacities) out.push(`- **${c.title} (${c.id})** — ${c.text}`);
out.push("\n");
out.push(h(3, "Capacity States"));
for (const s of pfi.pfiSystem.capacityStates) out.push(`- **${s.title} (${s.id})** — ${s.text}`);
out.push("\n");
out.push(h(3, "Periodização"));
out.push(bullets(pfi.pfiSystem.periodization));
out.push(h(3, "Adaptação"));
out.push(bullets(pfi.pfiSystem.adaptation));
